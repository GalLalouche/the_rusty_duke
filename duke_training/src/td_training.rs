...
/// TD trainer that plays games and trains the value network using temporal
/// difference learning (batch-mode, without eligibility traces for now).
pub struct TdTrainer<B: AutodiffBackend> {
    pub model: ValueNetwork<B>,
    optimizer: OptimizerAdaptor<Sgd<B::InnerBackend>, ValueNetwork<B>, B>,
    lr: f64,
    device: B::Device,
}
...
impl<B: AutodiffBackend> TdTrainer<B> {
    ...
    pub fn new(device: B::Device, lr: f64) -> Self {
        let model = ValueNetwork::new(&device);
        let optimizer = SgdConfig::new().init::<B, ValueNetwork<B>>();
        Self {
            model,
            optimizer,
            lr,
    ...
    /// `[batch, NUM_PLANES, BOARD_SIZE, BOARD_SIZE]`.
    fn encode_batch(&self, states: &[GameState]) -> Tensor<B, 4> {
        let tensors: Vec<Tensor<B, 3>> = states
            .iter()
            .map(|gs| encode_state::<B>(gs, &self.device))
            .collect();
