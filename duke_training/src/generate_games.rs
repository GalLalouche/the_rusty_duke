...
use duke_training::game_setup::{
    create_bag, create_initial_state, greedy_move_deep, MAX_TURNS,
};
        ...
        match game.game_result() {
            GameResult::Ongoing => {
                ...
                if rng.gen::<f64>() < epsilon {
                    ...
                    // Best negamax move at given depth
                    let mv = greedy_move_deep(&mut game, evaluator, depth, rng);
...
fn main() {
    ...
    (0..num_chunks).into_par_iter().for_each(|chunk_idx| {
        ...
        let mut chunk_games: Vec<(Vec<GameState>, GameResult)> = Vec::new();

        for _game_idx in chunk_start..chunk_end {
            let initial = create_initial_state(&bag);
            let (states, result) = play_depth_game(&initial, &evaluator, depth, &mut rng, epsilon);
            chunk_games.push((states, result));
        }

        // Write the chunk to disk under the lock
        {
            let mut w = writer.lock().unwrap();
            for (states, result) in &chunk_games {
                total_states.fetch_add(states.len() as u64, Ordering::Relaxed);
                w.write_game(&states, result).expect("Failed to write game");
            }
        }
    ...
    // Sync periodically is handled by chunks; finalize here
